...
import com.fag.infra.celcoin.dto.CelcoinProvidersDTO;
...
import com.fag.infra.celcoin.mappers.CelcoinProviderMapper;
...
public class RechargeCelcoin implements IRechargeVendor {
    ...
    @Override
    public List<ProviderDTO> listProviders(Integer stateCode, Integer category) {

        try {
            CelcoinProvidersDTO providers = restClient.listProviders(getToken(), stateCode, category);

            return providers.getProviders().stream()
                    .map(provider -> CelcoinProviderMapper.toAppDTO(provider))
